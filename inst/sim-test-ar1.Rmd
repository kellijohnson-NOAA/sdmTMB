...
#### Creat barrier polygons to test influece of a barrier mesh
```{r}
loc_xy <- cbind(grid$x, grid$y)

ConvHull <- INLA::inla.nonconvex.hull(loc_xy)
outer <- cbind(ConvHull$loc[,1], ConvHull$loc[,2])

FullPoly<- sp::Polygon(as.data.frame(outer), hole = FALSE)


x <- c(0, 0, 15, 15)
y <- c(15, 20, 20, 15)
Hole1 <- as.data.frame(cbind(x,y))

x <- c(15, 15, 20, 20)
y <- c(10, 15, 15, 10)
Hole2 <- as.data.frame(cbind(x,y))

Hole1Poly <- sp::Polygon(Hole1, hole = TRUE)
Hole1SP   <- sp::SpatialPolygons(list(sp::Polygons(list(Hole1Poly), ID = '1')))

Hole2Poly <- sp::Polygon(Hole2, hole = TRUE)
Hole2SP   <- sp::SpatialPolygons(list(sp::Polygons(list(Hole2Poly), ID = '2')))

AllPoly <- sp::SpatialPolygons(list(sp::Polygons(list(FullPoly, Hole1Poly, Hole2Poly), ID = '1')))
bound <- INLA::inla.sp2segment(AllPoly)

barrier_polygons <- raster::union(Hole1SP,Hole2SP)
#grid_polygon <- sp::SpatialPolygons(list(sp::Polygons(list(sp::Polygon(grid, hole = TRUE)), ID='0')))

#clipped <- raster::crop(grid_polygon, barrier_polygons)

hole1_points <- expand.grid(x = min(Hole1$x):max(Hole1$x), y = min(Hole1$y):max(Hole1$y))

hole2_points <- expand.grid(x = min(Hole2$x):max(Hole2$x), y = min(Hole2$y):max(Hole2$y))

holes <- rbind(hole1_points, hole2_points)

grid <- anti_join(grid, holes, by = c("x","y"))
...
#spde$spde$param.inla["M0"]
```
OR/
With barriers
```{r}
spde2 <- make_spde(x = dat$x, y = dat$y, n_knots = 40, boundary = bound)
plot_spde(spde2)
glimpse(spde2)
```
...
all_iter <- purrr::pmap(args, sim_predictions,
  grid = grid, x = grid$X, y = grid$Y, knots = 40,
  formula = observed ~ 1, family = gaussian(link = "identity")
)
```
