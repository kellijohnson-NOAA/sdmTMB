...
```{r}
grid <- expand.grid(x = seq(1:25), y = seq(1:25))
```
...
```{r, echo=FALSE}
g <- ggplot(simdat[[1]], aes_string("x", "y", fill = "z")) +
              geom_raster() +
              facet_wrap(~time) +
...
```{r, echo = FALSE}
g <- ggplot(dat, aes_string("x", "y", fill = "z")) +
              geom_raster() +
              facet_wrap(~time) +
              scale_fill_viridis_c() +
              coord_fixed()
...
With barriers
```{r}
spde <- make_spde(x = dat$x, y = dat$y, n_knots = 150, barrier = barrier_polygons)
plot_spde(spde)
```
...
**values** List of values to be shown in plotted on separate panels (Default = c("real_z", "est", "diff"))\
